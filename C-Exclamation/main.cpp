...
int main(int argc,char * argv[])
{
    ...
    memory.mem.push_back(CeVariable(RUNTIME_V,to_string(0),true,-999));
    start = timeGetTime();
    memory.mem.push_back(CeVariable("author__",info.authorName,true,-999));
    if(!singleLine)decode(filer);//解码
    else {
        string a = argv[2];
        code(a,0,"");
    }
...
string code(const string & f,int line,string blockOpition,PTIfStates ifStates,bool api,string varNameADD){
    ...
    if(title.compare("print") == 0){
        ...
        if(rst != ""){errMessage(varNotFound,line,index1,rst);return "";}
        toCeStr(cestr,subr,&rt);
        if(rt != -1){errMessage(oppCharNotHave,line,index1,to_string(f[rt]));return "";}
        cout << cestr;//don't need to add endl!!!!
        return to_string(cestr.length());
    }else if(title.compare("李辉发小学生") == 0){
        errMessage(noAcess,line,0,"我也这么觉得.");
        return "李辉发说:“焯李妈！”";
    }else if(title.compare("seek") == 0){
        for(const CeVariable & v : memory.mem){
            if(!v.name.compare(RUNTIME_V)){
                cout << RUNTIME_V << "    |    " << "[UNDEFINED]\n";
            }else cout << v.name << "    |    " << v.value << "\n";
        }
        return "";
    }else if(title.compare("shell") == 0){
        editMode = true;
        colorfulPrint("CE SHELL VER1.0\n",blue);
        string lines = "";
        string result = "";
        bool echo = true;
        while(true){
            if(echo)colorfulPrint("\n>>>",yellow);
            getline(cin,lines);
            if(lines[0] == '>'){
                system(lines.substr(1).c_str());
                continue;
            }else if(lines[0] == '/'){
                continue;
            }else if(!lines.compare(0,5,"@echo")){
                echo = !echo;
                continue;
            }
            result = code(lines,0,"");
            colorfulPrint(result,blue);
        }
        editMode = false;
        return "";
    }else if(title.compare("声明") == 0 || title.compare("var") == 0 || title.compare("string") == 0){
        index1 = index1 + 1;
        index2 = f.length();
        ...
        if(memory.has(varName)){errMessage(varReDefine,line,index1,varName);return "";}
        ...
        for(int i = 0;i < (int)sayings.size();i++){
            if(sayings[i].compare(constVar) == 0)isConst = true;
                ...
                else {errMessage(noAcess,line,index1,"INORECASE");return "";}
            }else if(sayings[i].compare(apiOutVar) == 0){
                if(useApi || !checkApi)isout = true;
                else {errMessage(noAcess,line,index1,"OUTABLE");return "";}
            }
        }
        var.isconst = isConst;
        ...
        if(!memory.has(subr) && subr.compare("") != 0){errMessage(varNotFound,line,index1,subr);return "";}
        string rc = "";
        char ch;
        while((ch = getchar()) != '\n'){
            rc += ch;
        ...
        if(subr.find('(') != string::npos)
            subr = code(f,line,blockOpition,ifStates,true,subr);
        string oldv = "";
        if(!memory.has(subr)){return "";}
        auto it = memory.mem.begin();
        for(;it < memory.mem.end();++it){
            if(!it->name.compare(subr)){
                oldv = it->value;
                break;
            }
        }
        ...
        if(subr.find('(') != string::npos)
            subr = code(f,line,blockOpition,ifStates,true,subr);
        if(!memory.has(subr)){errMessage(varNotFound,line,index1,subr);return "";}
        if(!memory.mem[memory.getIndex(subr)].out){errMessage(noAcess,line,index1, "使用local变量:"+memory.mem[memory.getIndex(subr)].name);return "";}
        string _v;
        _v.resize(1024);
        sprintf((char *)_v.c_str(),
               ...
               memory.getIndex(subr),(memory.mem[memory.getIndex(subr)].isconst?"是":"否"),
               (memory.mem[memory.getIndex(subr)].inorecase?"是":"否"),
        ...
        string cestr = "";
        string subr = f.substr(index1,index2 - index1);
        if(ceGetCharC(subr,',') != 2){errMessage(argNotEnough,line,index1,"3");return "";}
        vector<string> args;
        split(args,subr,',');
        ...
        if(ceGetCharC(args[2],'\"') != 0){errMessage(constCannotSet,line,index1,args[2]);return "";}
        if(!memory.has(args[2])){errMessage(varNotFound,line,index1,args[2]);return "";}
        ...
        int index1 = find(f,"(",1);
        if(index1 < 0){errMessage(isNotFunction,line,index1,"exitCmd");return "";}
        HWND win = GetConsoleWindow();
        PostMessage(win,WM_CLOSE,0,0);//直接关闭
    }else if(title.compare("exit") == 0){
        int index1 = find(f,"(",1);
        if(index1 < 0){errMessage(isNotFunction,line,index1,"exit");return "";}
        exit(0);
        ...
        int index1 = find(f,"(",1);
        if(index1 < 0){errMessage(isNotFunction,line,index1,"clear");return "";}
        system("CLS");
        ...
        if(rt != ""){errMessage(varNotFound,line,index1,rt);return "";}
        ...
        if(!memory.has(vname)){errMessage(varNotFound,line,index1,vname);return "";}
        time_t t = time(0);
        ...
        if(result == -1){errMessage(varNotFound,line,index1,errMsg);return "";}
        ...
        if(findAllSW(f,"+=") != string::npos){
            ...
            if(!memory.has(varName)){errMessage(varNotFound,line,index1,varName);return "";}
            ...
            if(varName.find(':') != string::npos)
                varName = code(f,line,blockOpition,ifStates,true,f);
            if(!memory.has(varName)){errMessage(varNotFound,line,index1,varName);return "";}
            if(!varName.compare(RUNTIME_V)){
                info.runTime = timeGetTime() - start;
                memory.mem[memory.getIndex(runtime_s)].setValue(to_string(info.runTime),true);
            }
